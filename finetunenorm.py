...
def replace_conv_with_lora(module, lora_config):
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
            setattr(module, name, LoRAConv2d(base_module=child, lora_config=lora_config))
        else:
...
def freeze_except_lora(module):
    ...
    for submodule in module.modules():
        if isinstance(submodule, LoRAConv2d):
            for param in submodule.parameters():
                param.requires_grad = True
    for name, param in module.named_parameters():
        if "base_module" in name:  # identifies the original conv weights
            param.requires_grad = False

def enable_lora_adapters(module):
    for submodule in module.modules():
        if isinstance(submodule, LoRAConv2d):
            submodule.enable_adapter()
