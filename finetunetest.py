...
from lora_modules import LoRAConv2d2
...
def replace_conv_with_lora(module, lora_config):
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
            setattr(module, name, LoRAConv2d2(base_module=child, lora_config=lora_config))
        else:
            replace_conv_with_lora(child, lora_config)

def freeze_except_lora(module):
    for param in module.parameters():
        param.requires_grad = False
    for submodule in module.modules():
        if isinstance(submodule, LoRAConv2d2):
...
def patch_first_conv2d_input_channels(module, new_in_channels):
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
...
def train(model, dataloader, optimizer, device, epochs=5, gain=5):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}")):
            ...
            criterion = RateDistortionLoss(lmbda=0.0067, metric="mse")
            loss_out = criterion(result, batch_pre, masks=masks)
...
if __name__ == "__main__":
    ...
    replace_conv_with_lora(model.g_a, lora_config)
    replace_conv_with_lora(model.h_a, lora_config)
    
    freeze_except_lora(model.g_a)
    freeze_except_lora(model.h_a)
    
    for name, param in model.named_parameters():
        if not (name.startswith("g_a") or name.startswith("h_a") or name.startswith("entropy_bottleneck")):
            param.requires_grad = False
